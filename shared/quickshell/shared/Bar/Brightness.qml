import Quickshell
import Quickshell.Wayland
import Quickshell.Io
import QtQuick
import QtQuick.Layouts
import QtQuick.Controls
import ".." as Root

Item {
  id: root
  property var panelWindow

  property bool menuOpen: false
  property bool popupVisible: false

  readonly property int animDuration: 250
  readonly property int autoHideDelay: 1000

  // Track if we triggered the brightness change ourselves
  property bool selfTriggered: false
  // Skip first brightness notification on startup
  property bool initialized: false
   
  // Brightness percentage 0-100
  property int brightnessPercent: 100
  property int previousBrightnessPercent: 100

  // Process for polling brightness
  Process {
    id: pollProcess
    command: ["sh", "-c", "echo $(brightnessctl get) $(brightnessctl max)"]
    stdout: StdioCollector {
      onStreamFinished: {
        const parts = text.trim().split(/\s+/)
        if (parts.length === 2) {
          const current = parseInt(parts[0])
          const max = parseInt(parts[1])
          if (!isNaN(current) && !isNaN(max) && max > 0) {
            const newPercent = Math.round((current / max) * 100)
            if (!initialized) {
              initialized = true
              brightnessPercent = newPercent
              previousBrightnessPercent = newPercent
              return
            }
            if (!selfTriggered) {
              // External change detected - only show menu if value actually changed
              brightnessPercent = newPercent
              if (newPercent !== previousBrightnessPercent) {
                previousBrightnessPercent = newPercent
                showTemporarily()
              }
            }
          }
        }
      }
    }
    onExited: {
      // Restart the polling process
      if (!running) {
        restartTimer.start()
      }
    }
  }

  Timer {
    id: restartTimer
    interval: 100
    onTriggered: {
      pollProcess.running = true
    }
  }

  // Start polling on load
  Component.onCompleted: {
    pollProcess.running = true
  }

  function showTemporarily() {
    menuOpen = true
    autoHideTimer.restart()
  }

  function setBrightness(percent: int) {
    selfTriggered = true
    Quickshell.execDetached(["brightnessctl", "set", percent + "%"])
    brightnessPercent = percent
    previousBrightnessPercent = percent
    // Reset selfTriggered after a short delay
    resetTimer.restart()
  }

  Timer {
    id: resetTimer
    interval: 300
    onTriggered: selfTriggered = false
  }

  onMenuOpenChanged: {
    if (menuOpen) {
      hideTimer.stop()
      popupVisible = true
      // Force immediate refresh
      pollProcess.running = true
    } else {
      autoHideTimer.stop()
      hideTimer.start()
    }
  }

  Timer {
    id: hideTimer
    interval: animDuration + 50
    onTriggered: popupVisible = false
  }

  Timer {
    id: autoHideTimer
    interval: autoHideDelay
    onTriggered: {
      if (!popup.mouseInside) {
        menuOpen = false
      }
    }
  }

  implicitWidth: brightnessButton.width
  implicitHeight: brightnessButton.height

  // Brightness button
  Rectangle {
    id: brightnessButton
    width: 24
    height: 24
    radius: 6
    color: buttonArea.containsMouse ? Root.Theme.surfaceHover : (menuOpen ? Root.Theme.surface : "transparent")

    Behavior on color {
      ColorAnimation { duration: 100 }
    }

    Image {
      anchors.centerIn: parent
      source: "../assets/brightness.svg"
      sourceSize.width: 14
      sourceSize.height: 14
    }

    MouseArea {
      id: buttonArea
      anchors.fill: parent
      hoverEnabled: true
      cursorShape: Qt.PointingHandCursor
      onClicked: menuOpen = !menuOpen
    }
  }

  // Brightness popup
  PopupWindow {
    id: popup
    anchor {
      window: panelWindow
      rect.x: panelWindow.width - popupContent.width - 8
      rect.y: brightnessButton.height + 12
      edges: Edges.Top | Edges.Left
    }

    visible: popupVisible

    implicitWidth: popupContent.width
    implicitHeight: popupContent.height

    color: "transparent"

    property bool mouseInside: false

    onVisibleChanged: {
      if (!visible) mouseInside = false
    }

    Rectangle {
      id: popupContent
      width: 200
      height: 56
      radius: 8
      color: Root.Theme.background
      border.color: Root.Theme.surface
      border.width: 1

      scale: menuOpen ? 1 : 0.95
      opacity: menuOpen ? 1 : 0
      transformOrigin: Item.TopRight

      Behavior on scale {
        NumberAnimation { duration: 250; easing.type: Easing.OutCubic }
      }
      Behavior on opacity {
        NumberAnimation { duration: 250; easing.type: Easing.OutCubic }
      }

      HoverHandler {
        id: contentHover
        onHoveredChanged: {
          if (hovered) {
            popup.mouseInside = true
            autoHideTimer.stop()
          } else if (popup.mouseInside) {
            popup.mouseInside = false
            autoHideTimer.restart()
          }
        }
      }

      RowLayout {
        anchors.fill: parent
        anchors.margins: 12
        spacing: 10

        // Sun icon
        Rectangle {
          width: 24
          height: 24
          radius: 4
          color: "transparent"

          Image {
            anchors.centerIn: parent
            source: "../assets/brightness.svg"
            sourceSize.width: 16
            sourceSize.height: 16
          }
        }

        // Brightness slider
        Slider {
          id: brightnessSlider
          Layout.fillWidth: true
          Layout.preferredHeight: 24

          from: 0
          to: 100
          value: brightnessPercent
          stepSize: 1

          onMoved: {
            setBrightness(value)
            autoHideTimer.restart()
          }

          background: Rectangle {
            x: brightnessSlider.leftPadding
            y: brightnessSlider.topPadding + brightnessSlider.availableHeight / 2 - height / 2
            width: brightnessSlider.availableWidth
            height: 6
            radius: 3
            color: Root.Theme.surface

            Rectangle {
              width: brightnessSlider.visualPosition * parent.width
              height: parent.height
              radius: 3
              color: Root.Theme.primary
            }
          }

          handle: Rectangle {
            x: brightnessSlider.leftPadding + brightnessSlider.visualPosition * (brightnessSlider.availableWidth - width)
            y: brightnessSlider.topPadding + brightnessSlider.availableHeight / 2 - height / 2
            width: 14
            height: 14
            radius: 7
            color: brightnessSlider.pressed ? Root.Theme.surfaceHover : Root.Theme.text
            border.color: Root.Theme.primary
            border.width: 2

            Behavior on color {
              ColorAnimation { duration: 100 }
            }
          }
        }

        // Percentage display
        Text {
          text: brightnessPercent + "%"
          color: Root.Theme.text
          font.pixelSize: 11
          font.family: Root.Theme.fontFamily
          Layout.preferredWidth: 32
          horizontalAlignment: Text.AlignRight
        }
      }
    }
  }
}
